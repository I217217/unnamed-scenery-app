...
### データ永続化について
- このアプリは **IndexedDB** をブラウザ内に作成してデータを保存します。
- ユーザーごとのデータは**各ユーザーのブラウザ**に保存され、サーバーには送信されません。
- つまり、デプロイ先のサーバーデータベースを用意する必要はありません（バックエンド不要）。

### メディアファイルの容量
- 画像や音声はBase64形式などでIndexedDBに保存されます。
- ブラウザのストレージ容量制限（通常はディスク空き容量の一定割合）に依存します。
